...
int MainWindow::saveProblem(void)
{
    ...
    if (save == true){
        ...
        status = bocopSave->writeFiles(ui->radioButton_multiMode);
        if (status != 0) {
...
/**
  *\fn int MainWindow::saveSingleBounds(void)
  * This method collects the values of the bounds found in model_bounds,
  * and saves it in bocopSave, for a future file writing.
  */
int MainWindow::saveSingleBounds(void)
{
    double lower, upper;
    string type;
...
/**
  *\fn int MainWindow::saveMultiBounds(void)
  * This method collects the values of the bounds found in model_multiBounds,
  * and saves it in bocopSave.
  */
//int MainWindow::saveMultiBounds(void)
//{
//    // If the model is empty, we don't save anything
//    if (model_bounds == 0) {
//        int stat = loadDefinitionBounds();
...
//    int dim_multi_ifcond = 0;
//    int dim_multi_state = 0;
//    int dim_multi_control = 0;
//    int dim_multi_algebraic = 0;
//    int dim_multi_optimvars = 0;
//    int dim_multi_pathcond = 0;

//    int index_multi_ifcond = 0;
//    int index_multi_state = 0;
//    int index_multi_control = 0;
//    int index_multi_algebraic = 0;
//    int index_multi_optimvars = 0;
//    int index_multi_pathcond = 0;

//    int nb_row = model_multiBounds->rowCount();

//    for (int i=0; i<nb_row; ++i) {

//        // We get the items on the current row :
//        QStandardItem* it_name  = model_multiBounds->item(i,0);
//        QStandardItem* it_lower = model_multiBounds->item(i,1);
//        QStandardItem* it_upper = model_multiBounds->item(i,2);
//        QStandardItem* it_type  = model_multiBounds->item(i,3);

//        // If the items were found, we read their texts :
//        if (it_name != 0 && it_lower != 0 && it_upper != 0 && it_type != 0) {
//            QString name_str  = it_name->text();
//            QString lower_str = it_lower->text();
//            QString upper_str = it_upper->text();
//            QString type_str  = it_type->text();

////            qDebug() << name_str << " " << lower_str << " " << upper_str << " " << type_str;

//            // If the text is not empty we add it to the lists :
//            if (!name_str.isEmpty() && !lower_str.isEmpty() && !upper_str.isEmpty() && !type_str.isEmpty()) {
//                if (name_str.at(0) < 'A' || name_str.at(0) > 'Z')
//                    bocopSave->setMultiBounds(name_str.toStdString(), lower_str.toStdString(), upper_str.toStdString(), type_str.toStdString());
//                else{
//                    if (name_str == "BoundaryCondition") index_multi_ifcond = i;
//                    else if (name_str == "State") index_multi_state = i;
//                    else if (name_str == "Control") index_multi_control = i;
//                    else if (name_str == "Algebraic") index_multi_algebraic = i;
//                    else if (name_str == "Optimisation") index_multi_optimvars = i;
//                    else if (name_str == "PathConstraint") index_multi_pathcond = i;
//                    else
//                        qDebug() << "Can not find " << name_str;
//                }
//            }
//        }
//    }
////    qDebug() << index_multi_ifcond << " " << index_multi_state << " " << index_multi_control << " "
////             << index_multi_algebraic << " " << index_multi_optimvars << " " << index_multi_pathcond;

//    dim_multi_ifcond    = index_multi_state - index_multi_ifcond-1;
//    dim_multi_state     = index_multi_control - index_multi_state-1;
//    dim_multi_control   = index_multi_algebraic - index_multi_control-1;
//    dim_multi_algebraic = index_multi_optimvars - index_multi_algebraic-1;
//    dim_multi_optimvars = index_multi_pathcond - index_multi_optimvars-1;
//    dim_multi_pathcond  = nb_row - index_multi_pathcond-1;

////    qDebug() << dim_multi_ifcond << " " << dim_multi_state << " " << dim_multi_control << " "
////             << dim_multi_algebraic << " " << dim_multi_optimvars << " " << dim_multi_pathcond;

//    bocopSave-> setDimMultiState(dim_multi_state);
//    bocopSave-> setDimMultiControl(dim_multi_control);
//    bocopSave-> setDimMultiAlgebraic(dim_multi_algebraic);
//    bocopSave-> setDimMultiOptimvars(dim_multi_optimvars);
//    bocopSave-> setDimMultiInitFinalCond(dim_multi_ifcond);
//    bocopSave-> setDimMultiPathConstraints(dim_multi_pathcond);

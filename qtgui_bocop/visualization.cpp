...
int MainWindow::addVariableBoundsOnPlot(int indice, QwtPlot *visu_qwtPlot, double time_min, double time_max)
{
    ...
    double lower_bound = bocopDef->lowerBoundVariable(indice);
    double upper_bound = bocopDef->upperBoundVariable(indice);
    string type = bocopDef->typeBoundVariable(indice);
...
{
    int ok = 1;
    double lower_bound = bocopSol->lowerBoundPathConstraint(indice);
    double upper_bound = bocopSol->upperBoundPathConstraint(indice);
    string type = bocopSol->typeBoundPathConstraint(indice);

    ok = addBoundsOnPlot(lower_bound, upper_bound, type, visu_qwtPlot, time_min, time_max);
    if (ok != 0)

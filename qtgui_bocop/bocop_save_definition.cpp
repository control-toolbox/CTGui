...
//    multiBounds_names.reserve(100);
//    multiBounds_lower.reserve(100);
//    multiBounds_upper.reserve(100);
//    multiBounds_type.reserve(100);
...
//    dim_state_multiBounds = -1;
//    dim_control_multiBounds = -1;
//    dim_algebraic_multiBounds = -1;
//    dim_optimvars_multiBounds = -1;
//    dim_ifcond_multiBounds = -1;
//    dim_pathcond_multiBounds= -1;
}
...
/**
  *\fn int BocopSaveDefinition::writeFiles(bounds_mode)
  ...
  * for each file.
  */
int BocopSaveDefinition::writeFiles(bool multiBounds_mode)
{
    int status;
...
int BocopSaveDefinition::writeBounds(void)
{
    ...
    // We get the indices of the block lines:
    vector<int> ind_block;
    ...
    for (i=0; i<(int)all_in_bounds.size(); ++i) {
        ...
        else if (all_in_bounds.at(i).find(">") != string::npos ){
            ind_block.push_back(i);
        }
    }

    const int ini_final_position = ind_commented.at(5);
    const int state_position = ind_commented.at(6);
    const int control_position = ind_commented.at(7);
    const int algebratic_position = ind_commented.at(8);
    const int opti_position = ind_commented.at(9);

    // if we have boundary block
    if (!ind_block.empty()){
        for (int i=0; i<ind_block.size(); i++){
            int position = ind_block.at(i)-1;
                // initial and final conditions
                if (position == ini_final_position){
                    if (old_dim_ifcond != dim_ifcond){
                        all_in_bounds.erase(all_in_bounds.begin()+ind_block.at(i));
                        // fill one by one
                        ...
                        for (i=0; i<dim_ifcond; ++i) {
                            ...
                            all_in_bounds.insert(all_in_bounds.begin()+position+1+i, stream.str());
                        }
                ...
                // state variables
                else if (position == state_position){
                    cout << "modify state" << endl;
                    if (old_dim_state != dim_state){
                        all_in_bounds.erase(all_in_bounds.begin()+ind_block.at(i));
                        cout << position << endl;
                        // fill one by one
                        int begin_vec = dim_state; // index where the ifcond begins in the bounds arrays
                ...
                // algebratic
                else if (position == algebratic_position){
                }

                // optimatimization
                else if ( position == opti_position){
                }
...
/**
  *\fn int BocopSaveDefinition::setMultiBounds(string name, string lower, string upper, string type)
  *
  */
int BocopSaveDefinition::setMultiBounds(string name, string lower, string upper, string type)
{
    multiBounds_names.push_back(name);
    multiBounds_lower.push_back(lower);
    multiBounds_upper.push_back(upper);
    multiBounds_type.push_back(type);
    return 0;
}


/**
  *\fn int BocopSaveDefinition::writeMultiBounds(void)
  * This method writes the multibounds saved in triplet arrays
  * <names,lower_bound,upper_bound,type_bound> into .bounds file.
  */
int BocopSaveDefinition::writeMultiBounds(void)
{

    if (multiBounds_names.size() <= 0 || multiBounds_lower.size() <= 0 ||
            multiBounds_upper.size() <= 0 || multiBounds_type.size() <=0 ) {
        m_errorString.append("BocopSaveDefinition::writeMultiBounds : error");
        m_errorString.append("MultiBounds array should not be empty...");
        return 1;
    }

    // First we read the existing file (if any) :
    string bounds_filename = problem_path + "problem.bounds";
    ofstream file_bounds(bounds_filename.c_str());
    ...
    // Now we can write the file with the new multi bounds values :
    string comment1 = "# This file contains all the bounds of your problem.";
    string comment2 = "# Bounds are stored in standard format :";
    string comment3 = "# [lower bound]  [upper bound] [type of bound]";

    file_bounds << comment1 << endl;
    file_bounds << comment2 << endl;
    file_bounds << comment3 << "\n" << endl;

    string comment4 = "# Dimensions (i&f conditions, y, u, z, p, path constraints) :";
    file_bounds << comment4 << endl;

    file_bounds << dim_ifcond << " "<< dim_state << " " << dim_control << " "
                << dim_algebraic << " " << dim_optimvars << " " << dim_pathcond << "\n" << endl;

    string comment5 = "# Bounds for the initial and final conditions :";
    file_bounds << comment5 << endl;

    // loop initial and final conditions
    int index = dim_ifcond_multiBounds;
    int p = 0;
    if (index != 0)
        for (int i = p; i <index ; i++)
            file_bounds << "> " << multiBounds_names.at(i) << " " << multiBounds_lower.at(i) << " "
                        << multiBounds_upper.at(i) << " " << multiBounds_type.at(i) << "\n";
    file_bounds << endl;

    string comment6 = "# Bounds for the state variables :";
    file_bounds << comment6 << endl;

    // loop state
    index += dim_state_multiBounds;
    p += dim_ifcond_multiBounds;
    for (int i = p; i <index; i++)
        file_bounds << "> " << multiBounds_names.at(i) << " " << multiBounds_lower.at(i) << " "
                        << multiBounds_upper.at(i) << " " << multiBounds_type.at(i) << "\n";
    ...
    string comment7 = "# Bounds for the control variables :";
    file_bounds << comment7 << endl;

    // loop control
    index += dim_control_multiBounds;
    p += dim_state_multiBounds;
    for (int i = p; i <index; i++)
        file_bounds << "> " << multiBounds_names.at(i) << " " << multiBounds_lower.at(i) << " "
                        << multiBounds_upper.at(i) << " " << multiBounds_type.at(i) << "\n";
    file_bounds << endl;

    string comment8 = "# Bounds for the algebraic variables :";
    file_bounds << comment8 << endl;
    // loop algebratic
    index += dim_algebraic_multiBounds;
    p += dim_control_multiBounds;
    ...
    string comment9 = "# Bounds for the optimization parameters :";
    file_bounds << comment9 << endl;

    // loop optimvars
    index += dim_optimvars_multiBounds;
    p += dim_algebraic_multiBounds;
    ...
    string comment10 = "# Bounds for the path constraints :";
    file_bounds << comment10 << endl;

    // loop path constraint
    index += dim_pathcond_multiBounds;
    p += dim_algebraic_multiBounds;

    for (int i = p; i <index; i++)
        file_bounds << "> " << multiBounds_names.at(i) << " " << multiBounds_lower.at(i) << " "
                        << multiBounds_upper.at(i) << " " << multiBounds_type.at(i) << endl;

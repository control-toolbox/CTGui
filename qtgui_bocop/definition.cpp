...
int MainWindow::loadDefinitionDimensions(void)
{
    ...
    if (ui->initialFinalConditionsDimensionLineEdit->text().isEmpty()) {
        value_str.setNum(bocopDef->dimBoundaryConditions());
...
QString MainWindow::nameInitFinalCond(const int i)
{
    ...
    if (bocopDef != 0)
        name = QString::fromStdString(bocopDef->nameBoundaryConditions(i)) ;
...
int MainWindow::dimInitFinalCond()
{
    ...
    if (!ok) {
        ...
        if (bocopDef != 0)
            return bocopDef->dimBoundaryConditions();
        else return -1;
    }
...
int MainWindow::loadDefinitionBounds(void)
{
    ...
    if (bocopDef != 0){
        flagBoundMode = bocopDef->flagMultiBoudarisMode();
    }

    //    if (flagBoundMode){

    //        ui->radioButton_multiMode->setChecked(true);
    //        ui->radioButton_multiMode->setEnabled(false);
    //        ui->radioButton_2->setEnabled(false);
    //        ui->bounds_treeView->hide();
    //        //ui->multiBound_tableView->show();

    //        if (bocopDef == 0)
    //            initializeDefinitionMultiBounds();

    //        if (model_multiBounds == 0){
    //            model_multiBounds = new QStandardItemModel();

    //            QVector<string> multiBoundBlock = QVector<string>::fromStdVector(bocopDef->getMultiBoundaryBlock());
    //            //            for (int i=0; i < multiBoundBlock.size(); i++)
    //            //                qDebug() << QString::fromStdString(multiBoundBlock.at(i));

    //            string variable_keys[6] = {"BoundaryCondition","State", "Control" , "Algebraic",
    //                                       "Optimisation" , "PathConstraint"};

    //            string name = variable_keys[0];
    //            string upper = " ";
    //            string lower = " ";
    //            string type = " ";
    //            int counter = 1;
    ...
    //            QStandardItem *state_line  = new QStandardItem(QString::fromStdString(name));
    //            QStandardItem *state_line1 = new QStandardItem(QString::fromStdString(lower));
    //            QStandardItem *state_line2 = new QStandardItem(QString::fromStdString(upper));
    //            QStandardItem *state_line3  = new QStandardItem(QString::fromStdString(type));

    //            state_line->setEditable(false);
    //            state_line1->setEditable(false);
    //            state_line2->setEditable(false);
    //            state_line3->setEditable(false);

    //            QList<QStandardItem*> first_line;
    //            first_line << state_line << state_line1 << state_line2 << state_line3;
    //            model_multiBounds->appendRow(first_line);

    //            // We fill the model with values from .bounds:
    //            for (int i=0; i<multiBoundBlock.size(); ++i) {

    //                string line = multiBoundBlock.at(i);
    //                stringstream ss(line);

    //                // add title line
    //                if (line[0] == '#' ){

    //                    name = variable_keys[counter];
    //                    upper = " ";
    //                    lower = " ";
    //                    type = " ";
    ...
    //                    QStandardItem *name_it  = new QStandardItem(QString::fromStdString(name));
    //                    QStandardItem *lower_it = new QStandardItem(QString::fromStdString(lower));
    //                    QStandardItem *upper_it = new QStandardItem(QString::fromStdString(upper));
    //                    QStandardItem *type_it  = new QStandardItem(QString::fromStdString(type));

    //                    name_it->setEditable(false);
    //                    lower_it->setEditable(false);
    //                    upper_it->setEditable(false);
    //                    type_it->setEditable(false);

    //                    QList<QStandardItem*> both;
    //                    both << name_it << lower_it << upper_it << type_it;

    //                    model_multiBounds->appendRow(both);
    //                }

    //                else{

    //                    ss >> name >> lower >> upper >> type;
    ...
    //        // We set four headers to the model :
    //        //model_multiBounds->setHorizontalHeaderItem( 0, new QStandardItem( "Name" ) );
    //        //model_multiBounds->setHorizontalHeaderItem( 1, new QStandardItem( "Lower Bound") );
    //        //model_multiBounds->setHorizontalHeaderItem( 2, new QStandardItem( "Upper Bound") );
    //        //model_multiBounds->setHorizontalHeaderItem( 3, new QStandardItem( "Type" ) );

    //        // Finally we pass the model to the tableview
    //        ui->multiBound_tableView->setModel(model_multiBounds);

    //        // We hide the numbers on the right :
    //        QHeaderView *vertical = ui->multiBound_tableView->verticalHeader();
    //        vertical->setContextMenuPolicy(Qt::CustomContextMenu);

    //        connect(vertical, SIGNAL(customContextMenuRequested(const QPoint&)),
    //                this, SLOT(multiBounds_ShowContextMenu()),Qt::UniqueConnection);

    //        // And we stretch the columns so that they fill the table view :
    //        QHeaderView *header = ui->multiBound_tableView->horizontalHeader();
    ...
    //        ui->multiBound_tableView->resizeColumnToContents(0);
    //        ui->multiBound_tableView->resizeColumnToContents(1);
    //        ui->multiBound_tableView->resizeColumnToContents(2);
    //        ui->multiBound_tableView->resizeColumnToContents(3);
    ...
    ui->radioButton_2->setEnabled(false);
    ui->multiBound_tableView->hide();
    ui->bounds_treeView->show();
            ...
            if (it->text() != "Constants") {
                ...
                for (int j=0;j<it->rowCount();++j) {
                    ...
                    else
                    {
                        ...
                        if (i<4) { // Variables
                            type = bocopDef->typeBoundVariable(add_dim.at(i) + j);
                            lower = bocopDef->lowerBoundVariable(add_dim.at(i)+j);
                            upper = bocopDef->upperBoundVariable(add_dim.at(i)+j);
                        }
                        else if (i == 4) { // Initial and final conditions
                            type = bocopDef->typeBoundBoundaryConditions(j);
                            lower = bocopDef->lowerBoundBoundaryConditions(j);
                            upper = bocopDef->upperBoundBoundaryConditions(j);
                        }
                        else if (i == 5) { // Path constraints
                            type = bocopDef->typeBoundPathConstraint(j);
                            lower = bocopDef->lowerBoundPathConstraint(j);
                            upper = bocopDef->upperBoundPathConstraint(j);
                        }
                    }
...
int MainWindow::loadDefinitionConstants(void)
{
    ...
    for (int i=0; i<dimConstants(); ++i)
    {
        if  (bocopDef == 0)
            ...
            constants_val <<  bocopDef->getConstant(i);
...
/**
  *\fn int MainWindow::multiBounds_ShowContextMenu(const QPoint& pos)
  * This method creates menu to add/delete block in multiBounds table.
  */
//void MainWindow::multiBounds_ShowContextMenu()
//{

//    QMenu myMenu;
//    myMenu.addAction("Add Item");
//    myMenu.addAction("Delete Item");

//    QObject::connect(&myMenu, SIGNAL(triggered(QAction*)), this, SLOT(multiBounds_menuSelection(QAction*)));
...
/**
  *\fn int MainWindow::multiBounds_menuSelection(QAction* action)
  * This method handle the actions of the menu to add/delete block in multiBounds table
  * We protect the title?
  */
//void MainWindow::multiBounds_menuSelection(QAction* action)
//{
//    QString my_action = action->text();
//    int index = ui->multiBound_tableView->currentIndex().row();

//    //qDebug() << "multiBounds Triggered: " << action->text();

//    if (my_action == "Add Item")
//    {
//        add_multiBounds_item(index+1);
//    }
//    else
//    {
//        // check if it is in the  title line
//        QString text =  (ui->multiBound_tableView->model()->data(ui->multiBound_tableView->model()->index(index,0)).toString());
//        QChar first_letter =  text.at(0);

//        if (first_letter < 'Z' && first_letter >= 'A'){
//            QMessageBox::warning(this, "Warning", "Do not delete the label line!");
//        }
//        else
//            delete_multiBounds_item(index);
...
/**
  *\fn void MainWindow::add_multiBounds_item(int index)
  * It allows users to add a line in the
  * model for multi bound.
  */
//void MainWindow::add_multiBounds_item(int index)
//{

//    QStandardItem *name_it  = new QStandardItem();
//    QStandardItem *lower_it = new QStandardItem();
//    QStandardItem *upper_it = new QStandardItem();
//    QStandardItem *type_it  = new QStandardItem();

//    QList<QStandardItem*> both;
//    both << name_it << lower_it << upper_it << type_it;

//    model_multiBounds->insertRow(index, both);
//    connect(model_multiBounds, SIGNAL(itemChanged(QStandardItem*)),
//            ui->multiBound_tableView,SLOT(resizeColumnsToContents()));
//    ui->multiBound_tableView->scrollToBottom();
//}


/**
  *\fn void MainWindow::on_delete_multiBounds_item(int index)
  * It allows users to delete a choisen line in the
  * model for multi bound.
  */
//void MainWindow::delete_multiBounds_item(int index)
//{
//    model_multiBounds->removeRow(index);
//    connect(model_multiBounds, SIGNAL(itemChanged(QStandardItem*)),
//            ui->multiBound_tableView,SLOT(resizeColumnsToContents()));
//}
...
/**
  *\fn int MainWindow::initializeDefinitionMultiBounds()
  * This method initializes the bounds model values. It reads current problem
  * .bounds file, and sets the values that will later be displayed in the
  * tableview.
  */
//int MainWindow::initializeDefinitionMultiBounds()
//{
//    model_multiBounds = new QStandardItemModel();

//    QStringList default_names;
//    default_names << "BoundaryCondition" << "State" <<  "Control" << "Algebraic"
//                  <<"Optimisation" <<  "PathConstraint";
...
//    // We get the names, we go through all the categories in the names model :
//    for (int i=0; i<6; i++) {
//        name = default_names.at(i);

//        QStandardItem *state_line  = new QStandardItem(name);
//        QStandardItem *state_line1 = new QStandardItem(QString::fromStdString(lower));
//        QStandardItem *state_line2 = new QStandardItem(QString::fromStdString(upper));
//        QStandardItem *state_line3  = new QStandardItem(QString::fromStdString(type));
...
//    ui->multiBound_tableView->resizeColumnsToContents();
...
/**
  *\fn int MainWindow::askBoundOptions(void)
  * This method ask you to choise your bound option when you create a new problem.
  */
void MainWindow::askBoundOptions(void)
{
    QMessageBox dialog_Bound;
    dialog_Bound.setWindowTitle("Choise bounds options!");
    //dialog_Bound.setText(".............");
    QRadioButton *radio1 = new QRadioButton(tr("One line per bound"), &dialog_Bound);
    QRadioButton *radio2 = new QRadioButton(tr("Bound block(see userguide)"), &dialog_Bound);
    QAbstractButton *Yes = dialog_Bound.addButton(tr("Ok"), QMessageBox::YesRole);

    radio1->move(15,0);
    radio2->move(15,20);
    dialog_Bound.resize(300,500);
    dialog_Bound.setSizePolicy(QSizePolicy::Expanding,QSizePolicy::Expanding);
    dialog_Bound.setStyleSheet("QRadioButton{min-width:500 px; font-size: 15px;}");
    radio1->setChecked(true);
    radio2->setChecked(false);
    dialog_Bound.exec();

    if (radio2->isChecked() && dialog_Bound.clickedButton()==Yes)
        flagBoundMode = 1; // set default mode
    else
        flagBoundMode = 0;

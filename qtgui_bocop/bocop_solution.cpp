...
// Authors: Vincent Grelard, Daphne Giorgi, Virgile Andreani, Jinyan Liu
...
int BocopSolution::allocateVectors(void)
{
    ...
    bound_pathcond_type = new string [dim_pathcond];
    for (int i=0; i<dim_pathcond; ++i) {
        //bound_pathcond[i] = new double[3];
        bound_pathcond[i] = new double[2];
    }
...
int BocopSolution::readVariablesAndConstraints(void)
{
    ...
    try {
        for (i=0; i<dim_pathcond; ++i) {
            ...
            ifile >> bound_pathcond[i][0] >> bound_pathcond[i][1] >> bound_pathcond_type[i];

...
int MainWindow::loadOptimization(void)
{
    ...
    // We click right to show the menu :
    QHeaderView *vertical = ui->ipopt_tableView->verticalHeader();
    vertical->setContextMenuPolicy(Qt::CustomContextMenu);

    connect(vertical, SIGNAL(customContextMenuRequested(const QPoint&)),
        this, SLOT(Ipopt_ShowContextMenu()),Qt::UniqueConnection);
...
/**
  *\fn void MainWindow::add_ipopt_item(int index)
  * It allows users to add a line in the
  * model for ipopt options.
  */
void MainWindow::add_ipopt_item(int index)
{
    QStandardItem *name_it = new QStandardItem();
    QStandardItem *value_it = new QStandardItem();
    ...
    QList<QStandardItem*> both;
    both << name_it << value_it;
    model_ipopt->insertRow(index, both);
    ui->ipopt_tableView->scrollToBottom();
}


/**
  *\fn void MainWindow::delete_ipopt_item(int index)
  * It allows users to delete a choisen line in the
  * model for ipopt options.
  */
void MainWindow::delete_ipopt_item(int index)
{
    model_ipopt->removeRow(index);
}
...
/**
  *\fn void MainWindow::Ipopt_menuSelection(QAction* action)
  * This slot is to handle add/delete action in ipopt option.
  */
void MainWindow::Ipopt_menuSelection(QAction* action)
{
    QString my_action = action->text();
    QModelIndex myIndex = ui->ipopt_tableView->currentIndex();
    int index = myIndex.row();

  if (my_action == "Add Item")
  {
      add_ipopt_item(index+1);
  }
  else
  {
      delete_ipopt_item(index);
  }

}

/**
  *\fn void MainWindow::Ipopt_ShowContextMenu()
  * This slot is to show the menu for add/delete item in ipopt option.
  */
void MainWindow::Ipopt_ShowContextMenu(){

    QMenu myMenu;
    myMenu.addAction("Add Item");
    myMenu.addAction("Delete Item");

    connect(&myMenu, SIGNAL(triggered(QAction*)), this, SLOT(Ipopt_menuSelection(QAction*)));
